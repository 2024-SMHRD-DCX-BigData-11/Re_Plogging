...
<div class="comRead-container">
    <div class="comRead-innner">
        ...
        <div class="comments-section">
            ...
            <!-- 댓글 최신순 정렬 -->
            <%
                List<Comment> comments = (List<Comment>)request.getAttribute("communityComments");
                if (comments != null) {
                    Collections.sort(comments, new Comparator<Comment>() {
                        public int compare(Comment c1, Comment c2) {
                            return c2.getIndate().compareTo(c1.getIndate()); // 최신순으로 정렬
                        }
                    });
                }
                request.setAttribute("sortedComments", comments);
            %>

            <!-- Comment List -->
            <c:forEach var="comment" items="${sortedComments}">
                <div class="comment">
                    <p><strong>${comment.user.userNick}</strong> - 
                    <fmt:formatDate value="${comment.indate}" pattern="yyyy-MM-dd HH:mm:ss" /></p>
                    <p>${comment.message}</p>
