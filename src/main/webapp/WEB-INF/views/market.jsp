		...
		<div class="market_List_wrap">
			...
			<ul class="market_pagination">
				...
				<c:if test="${currentPage > 1}">
					<li><a
						href="?page=${currentPage - 1}&size=8&sort=${currentSort}&category=${selectedCategory}">&laquo; </a></li>
				</c:if>

				<!-- 페이지 번호 목록 -->
				<c:forEach var="i" begin="1" end="${totalPages}">
					<li><a class="${currentPage == i ? 'active' : ''}"
						href="?page=${i}&size=8&sort=${currentSort}&category=${selectedCategory}">
							${i} </a></li>
				</c:forEach>
				...
				<c:if test="${currentPage < totalPages}">
					<li><a
						href="?page=${currentPage + 1}&size=8&sort=${currentSort}&category=${selectedCategory}">&raquo; </a></li>
				</c:if>
			</ul>
